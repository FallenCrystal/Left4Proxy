...
- **动态端点发现与选路**: 客户端通过服务端广播的端点列表 (`public_ips`)，自动探测局域网 (LAN) 直连与服务器中继线路，并根据 RTT 延迟与网络类型完成无缝路由切换；STUN 打洞辅助反射发现公网地址并维持 NAT 映射。
- **NAT 打洞直连**: 当代理服务端处于 NAT 后（无端口映射，仅靠 frp 中继暴露）时，客户端通过公网 STUN 服务器反射出自己的打洞端点并告知服务端，双方互打洞建立一条**不经过 frp 中继**的直接 UDP 通路；打洞失败（如对称型 NAT）时自动回落中继。
...
3. **并行探测与路由选择**：客户端对所有候选 IP 进行并行 UDP 心跳探测，测定 RTT 延迟。优先选择局域网内网 IP（`PathLAN`），其次按 RTT 选择最优的直连/中继候选。路径分为四类：
   - `PathLAN`：同局域网直连（最短路径）
   - `PathDirect`：客户端直连到有公网 IP 的服务器（真直连，无需打洞）
   - `PathPunch`：客户端经打洞/端口映射直连到 NAT 后面的服务器
   - `PathRelay`：客户端经内网穿透 (frp/HAProxy) 隧道中转
   服务器在握手时根据"该连接是否走隧道 (PROXY)"和"服务器自身是否在 NAT 后"判定路径类型，随握手响应告知客户端，客户端据此如实标记，不再猜测。握手响应还会携带服务端的公网打洞端点（`stun_server` 反射发现，或用 `punch_addr` 手动指定）。
4. **打洞直连（可选）**：若服务端在 NAT 后且客户端目前只能走中继，客户端会创建打洞套接字：用公网 STUN 服务器反射出该套接字的公网端点，通过中继把该端点告知服务端，服务端据此向客户端打洞（打开自身 NAT 映射），双方互发 STUN 探测。探测互通即确认直连通路，客户端按 RTT 把数据迁移到打洞通路，中继保留为自动回退。
5. **报文代理转发**: 本地 L4D2 客户端（`127.0.0.2:27015`）发出的游戏数据包经 Left4Proxy 封装后发送至最优服务端/内网穿透候选节点，服务端解包并转发至实际 L4D2 目标服务器 (`x.x.x.x:27015`)

## 部署操作

1. 部署一个 L4D2 专用服务器. 端口保持 `27015`, ip 开放 `0.0.0.0`.
2. (可选) 部署内网穿透. 远程端口应等于 Left4Proxy 服务端的 `listen_addr` 端口 (默认 `27014`), 而不是 L4D2 的 27015; 转发目标为 Left4Proxy 服务端. tcp+udp 同端口, 同远程对等. 若支持传递真实 IP, 在内网穿透上启用 Proxy Protocol v2 协议.
3. 部署 Left4Proxy Server. `public_ips` 内添加出口地址:  
   如果你的服务器和内网处于同一局域网. 添加内网 ip `192.168.x.x:27014`  
   如果你部署了内网穿透. 添加外部内网穿透的 ip/域名和端口.  
   警告: 如果开启了 `proxy_protocol_v2`, 不要暴露给未开启此功能的内网穿透. 这可能会导致 IP Spoof.
4. 客户端打开 Left4Proxy Client, 在 `server_addrs` 中填内网穿透外部地址. 验证是否可连接  
   如果服务器侧还配置了内网 ip 且你和服务器在同一内网, 它应该会自动切换到内网 IP.
5. 启动 Left4Proxy 的服务器和客户端, 求生之路专用服务器和求生之路客户端. 客户端控制台输入 `connect 127.0.0.2` 测试正常连接.

## 打洞直连（绕过 frp 中继）

当代理服务端在 NAT 后且没有端口映射时，玩家流量默认全部经 frp 中继绕行。开启打洞后，客户端会尝试与 NAT 后的服务端建立一条不经过 frp 的**直接** UDP 通路（打洞失败如对称型 NAT 时自动回落中继）。
...
1. 确保求生之路服务器侧配置了 `net_public_adr` 为 `127.0.0.2:27015` 或其它保留地址 (不写端口时默认 27015).
...
- `nat`: 服务器是否在 NAT 后。`auto` 自动检测（看有没有公网接口 IP），可显式设 `true`/`false` 覆盖（如云 VPS 只有内网 VPC 地址时）。影响直连客户端被标记为 `PathDirect`（有公网 IP）还是 `PathPunch`（NAT 后打洞/端口映射）。
